...
> Sharing of data between between many components, or when it needs to be passed through intermediate components. It helps keep state cleaner.

- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

> * Action creators - Functions that create objects (actions)
> * Actions - Objects that have a type and a payload. They get passed to the reducers.
> * Reducers - State-machine-liked functions that handle actions according to their type.
>  Only reducers can update the state. And they cannot mutate it, only return a modified version of it to go back to the store.
> * The store is a 'single source of truth' in that it is shared globally and immutable.

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

> * Application state is shared between all components in the application.
> * Component state is only visible from within itself.
> * For items that do not need to be shared and are not updated by outside components, they can be kept within their own components. This helps keep the application state to a more managable size.

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

> `Redux-thunk` is middleware. A `thunk` is a doubly-called function.
> Middleware intercepts actions before they get to their reducers and can modify/redistribute them.

- [x] What is your favorite state management system you've learned and this sprint? Please explain why!

> I was almost certain it was context API because it's so much quicker to set up for most projects. But as I see the power of the Redux tools (to view the entire state and be able to rewind), I'm starting to change that opinion.
